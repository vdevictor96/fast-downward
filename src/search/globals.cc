...
void save_plan(const vector<const Operator *> &plan, int iter)
{
    ...
    assert(is_plan_valid(plan));
}

bool is_plan_valid(const std::vector<const Operator *> &plan)
{
    State state = g_initial_state();
    for (unsigned i = 0; i < plan.size(); i++) {
        if (!plan[i]->is_applicable(state)) {
#ifndef NDEBUG
            std::cerr << "Plan is not valid!" << std::endl;
            std::cerr << "Cannot apply action " << plan[i]->get_name() << " to state {";
            for (unsigned var = 0; var < g_variable_domain.size(); var++) {
                if (var > 0) {
                    std::cerr << ", ";
                }
                std::cerr << g_fact_names[var][state[var]];
            }
            std::cerr << "}" << std::endl;
#endif
            return false;
    ...
    if (!test_goal(state)) {
        std::cerr << "Plan is not valid!" << std::endl;
        std::cerr <<
                  "Applying this action sequence to the initial state results in state {";
        for (unsigned var = 0; var < g_variable_domain.size(); var++) {
            if (var > 0) {
                std::cerr << ", ";
            }
        ...
        std::cerr << "} (which is not a goal state)." << std::endl;
    }
#endif
